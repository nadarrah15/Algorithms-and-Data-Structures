...
public class BinaryTree<E>
{
    ...
    public boolean equals(BinaryTree<E> other) {
    	...
    	//bitwise since isEmpty() returns booleans; makes this slighty faster 
        if (this.isEmpty() & other.isEmpty()) {
            return true;
        }
        //bitwise to make this run faster
        else if (this.isEmpty() ^ other.isEmpty()) {
            return false;
        }
        
        return this.value().equals(other.value()) && this.left().equals(other.left()) && this.right().equals(other.right());
    }
    ...
    public BinaryTree<E> copy(){	
    	...
    	return new BinaryTree<E>(this.value(), this.left().copy(), this.right().copy());
    }
    
    //Homework I dont get why this is static
    static boolean isAVL(BinaryTree tree){
    	if(tree.isEmpty())
    		return true;
    	
    	return (Math.abs(tree.left().height() - tree.right().height()) > 1) && isAVL(tree.left()) && isAVL(tree.right());
    	//or just return tree.isBalanced()
    	//(╯°□°）╯︵┻━┻
    	//PUT THE TABLE BACK BUBBA
    	//┬─┬ノ( º _ ºノ)
    }
